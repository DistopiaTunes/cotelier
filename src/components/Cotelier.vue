...
/* const workshops = [
    ...
    },
    {
        ...
        description: 'Desperte seu talento e aprimore suas habilidades no universo das caricaturas.',
        details: 'Aprenda a transformar retratos em caricaturas marcantes, explorando formas, exagero, linhas, esboço e finalização. Você entenderá como destacar traços, criar expressões divertidas e desenvolver seu próprio estilo artístico.',
        additionalInfo: 'Ideal para iniciantes e intermediários que desejam evoluir no desenho e explorar novas possibilidades criativas.',
    ...
    },
    {
        ...
        description: 'Descubra o universo do crochê e desenvolva suas habilidades manuais.',
...
const teachers = {
    ...
    2: {
        ...
        role: 'Artista multidisciplinar e Professor',
        bio: 'Raphael é mestre em pintura pela UNESPAR, atua há mais de 10 anos explorando, experimentando e ensinando diversas técnicas e materiais nas artes visuais. Sua abordagem une fundamentos clássicos a práticas atuais, com foco em inovação e investigação artística.',
    ...
    3: {
        ...
        bio: 'Mônica Lara é uma artista plástica, tatuadora e professora de artes visuais. Sua abordagem é contemporânea e inovadora, sempre buscando novas formas de inspirar seus alunos. Possui experiência com diversas linguagens, técnicas mistas e projetos artísticos autorais, conectando criatividade e sensibilidade ao estimular o desenvolvimento pessoal por meio da arte.',
    ...
    5: {
        ...
        instagram: 'https://instagram.com/alee_ilustra',
        instagramLabel: '@alee_ilustra/'
    },
    6: {
        name: 'Luciana Kotaka',
    ...
    8: {
        ...
        bio: 'Diego Nascimento é artista plástico e professor, explorando pintura e a representação da natureza desde a infância. Desde cedo, encontrou na pintura uma forma de observar o mundo e expressar sentimentos, inspirando-se especialmente na beleza e delicadeza das flores do jardim de sua infância.',
...
// Add method to open workshop dialog
/* const openWorkshopDialog = (index) => {
...
<div class="bg-stone-50">
    ...
    <div v-if="showWorkshopDialog" class="fixed inset-0 bg-black bg-opacity-75 z-[100] flex flex-col items-center justify-center" @click="closeWorkshopDialog">
                    ...
                    <transition name="fade" mode="out-in">
                        <div :key="currentWorkshopIndex">
                            ...
                            <div class="flex flex-col md:flex-row items-center md:items-stretch md:justify-between gap-3 mb-6">
                                <div class="bg-stone-100 border border-stone-300 rounded-lg px-4 py-2 shadow-sm w-full md:w-auto text-center">
                                    <span 
        ...
        <div class="mb-4 flex justify-center items-center">
            <a :href="`https://wa.me/554130492413?text=${encodeURIComponent(workshops[currentWorkshopIndex].whatsappMessage)}`" target="_blank" class="bg-green-500/90 hover:bg-green-600/90 text-white px-14 py-5 rounded-full flex items-center justify-center text-base md:text-lg w-fit relative glow-btn">
            ...
            <div class="max-w-6xl mx-auto px-4 py-4">
                <div class="flex items-center justify-between">
                    ...
                    <div class="hidden md:flex space-x-8">
                        <!-- <a href="#workshops" class="font-lora text-stone-600 hover:text-stone-900 font-bold">Oficinas</a> -->
                ...
                <div v-if="mobileMenuOpen" class="md:hidden pt-4 pb-2">
                    <div class="flex flex-col space-y-3">
                        <!-- <a href="#workshops" @click="toggleMobileMenu" class="font-lora text-stone-600 hover:text-stone-900 font-bold">Oficinas</a> -->
    ...
    <!-- Workshops Section -->
    <!--
    <section id="workshops" class="bg-white py-20 px-4 md:px-8">
        <div class="max-w-6xl mx-auto">
            <h2 class="font-lora text-4xl md:text-5xl text-center mb-16 text-stone-800">Oficinas de Férias</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <div v-for="(workshop, index) in workshops" :key="index" @click="openWorkshopDialog(index)" class="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow flex flex-col h-full cursor-pointer">
                    <div class="w-full h-64 relative overflow-hidden flex-shrink-0">
                        <img :src="workshop.image" :alt="workshop.title" class="w-full h-full object-cover">
                        <div v-if="workshop.teacherIds && workshop.teacherIds.length > 0" class="absolute bottom-4 left-4 flex items-center gap-2">
                            <div v-for="teacherId in workshop.teacherIds" :key="teacherId" class="flex items-center gap-2 bg-white/90 backdrop-blur-sm rounded-full px-1 py-1 shadow-lg">
                                <img 
                                    :src="teachers[teacherId]?.image" 
                                    :alt="teachers[teacherId]?.name"
                                    class="w-12 h-12 rounded-full object-cover flex-shrink-0"
                                />
                                <p class="text-stone-700 text-xs font-lora font-semibold pr-2">{{ teachers[teacherId]?.name }}</p>
                            </div>
                        </div>
                    </div>
                    
                    <div class="p-6 flex flex-col flex-grow">
                        <h3 class="font-lora text-2xl md:text-3xl text-stone-800 mb-3">{{ workshop.title }}</h3>
                        <p class="font-lora text-lg text-stone-600 mb-3">
                        ...
                        <div class="mt-auto flex flex-col gap-3 mb-6">
                            <div class="bg-stone-100 border border-stone-300 rounded-lg px-4 py-2 shadow-sm text-center">
                                <span class="text-stone-800 text-base font-lora font-bold">{{ workshop.duration || '3h' }}</span>
                            </div>
                            <div class="bg-stone-100 border border-stone-300 rounded-lg px-4 py-2 shadow-sm text-center">
                                <span class="text-stone-800 text-base font-lora font-bold">{{ workshop.price || 'R$ 100' }}</span>
                            </div>
                            <div class="bg-stone-100 border border-stone-300 rounded-lg px-4 py-2 shadow-sm text-center">
                                <span class="text-stone-800 text-base font-lora font-bold">{{ workshop.dateTime || '15 Jan - 14h' }}</span>
                            </div>
                        </div>
                        
                        <a :href="`https://wa.me/554130492413?text=${encodeURIComponent(workshop.whatsappMessage)}`" target="_blank" @click.stop class="bg-green-500/90 hover:bg-green-600/90 text-white px-6 py-3 rounded-full flex items-center justify-center text-base w-full">
                            <svg class="w-5 h-5 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                            </svg>
        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div v-for="(teacher, id) in teachers" :key="id" class="text-center">
                    ...
                    <p class="font-lora text-lg text-stone-500 mb-2 text-justify">{{ teacher.bio }}</p>
            ...
            <div class="bg-white p-8 rounded-lg max-w-2xl w-full mx-4" @click.stop>
                ...
                <div class="flex flex-col md:flex-row gap-8">
                    ...
                    <div>
                        ...
                        <p class="font-cormorant text-lg text-stone-500 mb-6 text-justify">{{ currentTeacher?.fullBio }}</p>
    ...
    <section id="schedule" class="py-20 px-4 md:px-8 bg-stone-50">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm relative">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="font-lora text-2xl text-stone-800">Oficina Livre de Pintura em Tela</h3>
                        <div class="flex items-center gap-1">
                            <svg class="w-6 h-6 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                    ...
                    <p class="font-lora text-xl text-stone-600 mb-4">Terças-feiras das <b><u>18:30 às 20:00h</u></b>- Faixa etária: a partir de 8 anos</p>
                    <p class="font-lora text-lg text-stone-500 mb-4">Ideal para quem deseja desenvolver ou aprimorar suas habilidades em pintura. Aprenda técnicas, explore materiais e aprofunde seus conhecimentos para ampliar sua expressão artística na pintura.</p>
              
                    <div class="flex flex-col md:flex-row gap-4 items-center">
                        <a href="https://wa.me/554130492413?text=Olá! Gostaria de informações sobre o curso do Prof. Diego nas terças-feiras" target="_blank" class="bg-green-500/80 hover:bg-green-600/80 text-white px-4 py-2 rounded-lg flex items-center justify-center text-sm w-fit">
                            <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                            </svg>
                            Agende uma aula experimental
                        ...
                        <div class="flex md:absolute md:bottom-8 md:right-8 items-center gap-2 bg-white/90 backdrop-blur-sm rounded-lg px-4 py-2">
                            <img 
                                :src="teachers[8]?.image" 
                                :alt="teachers[8]?.name"
                                class="w-5 h-5 rounded-full object-cover flex-shrink-0"
                            />
                            <p class="text-stone-700 text-sm font-lora font-semibold">{{ teachers[8]?.name }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm relative">
                    ...
                    <p class="font-lora text-lg text-stone-500 mb-4">Perfeito para quem está começando sua jornada artística. Aprenda técnicas fundamentais de desenho e pintura, luz e sombra, proporção, teoria das cores e mais.</p>
                    <div class="flex flex-col md:flex-row gap-4 items-center">
                        <a href="https://wa.me/554130492413?text=Olá! Gostaria de informações sobre o curso Oficina Kids da Prof. Márcia nas quartas-feiras" target="_blank" class="bg-green-500/80 hover:bg-green-600/80 text-white px-4 py-2 rounded-lg flex items-center justify-center text-sm w-fit">
                            <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                            </svg>
                            Agende uma aula experimental
            ...
            <div class="bg-white p-8 rounded-lg shadow-sm relative">
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm relative">
                    ...
                    <div class="flex flex-col md:flex-row gap-4 items-center">
                        <a href="https://wa.me/554130492413?text=Olá! Gostaria de informações sobre o Ateliê Multidisciplinar com o Prof. Raphael" target="_blank" class="bg-green-500/80 hover:bg-green-600/80 text-white px-4 py-2 rounded-lg flex items-center justify-center text-sm w-fit">
                            <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                            </svg>
                            Agende uma aula experimental
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm relative">
                    ...
                    <div class="flex flex-col md:flex-row gap-4 items-center">
                        <a href="https://wa.me/554130492413?text=Olá! Gostaria de informações sobre o Curso de Anime e Mangá com o Prof. Carlos" target="_blank" class="bg-green-500/80 hover:bg-green-600/80 text-white px-4 py-2 rounded-lg flex items-center justify-center text-sm w-fit">
                            <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                            </svg>
                            Agende uma aula experimental
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm relative">
                    ...
                    <p class="font-lora text-lg text-stone-500 mb-4">Curso ideal para quem deseja desenvolver ilustração artística em diferentes técnicas.
                        ...
                        <div class="flex flex-col md:flex-row gap-4 items-center">
                        <a href="https://wa.me/554130492413?text=Olá! Gostaria de informações sobre a Oficina de Ilustração" target="_blank" class="bg-green-500/80 hover:bg-green-600/80 text-white px-4 py-2 rounded-lg flex items-center justify-center text-sm w-fit">
                            <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                            </svg>
                            Agende uma aula experimental
...
.glow-btn {
    box-shadow: 0 0 4px 1px rgba(34, 197, 94, 0.3), 0 0 12px 3px rgba(22, 163, 74, 0.2);
...
.glow-btn:hover {
    animation: none;
    box-shadow: 0 0 8px 4px rgba(34, 197, 94, 0.3), 0 0 12px 8px rgba(22, 163, 74, 0.2);
}
